...
real_output_path="${output_path}real_tru/"
real_output_path_best="${output_path}real_tru/"
...
echo "java gelations.RunPrioritizer $data_format $mut_operator $cross_operator $select_operator $fit_transform $mut_rate $child_rep $pop_size $max_time $max_fitness $max_stag $metric $real_data_path$real_config${real_types[1]} $real_data_path$real_config${real_types[0]} $real_data_path$real_config${real_types[2]} $real_output_path_best$master_datafile${real_types[3]} $rep"

java gelations.RunPrioritizer $data_format $mut_operator $cross_operator $select_operator $fit_transform $mut_rate $child_rep $pop_size $max_time $max_fitness $max_stag $metric $real_data_path$real_config${real_types[1]} $real_data_path$real_config${real_types[0]} $real_data_path$real_config${real_types[2]} $real_output_path_best$master_datafile${real_types[3]} $rep

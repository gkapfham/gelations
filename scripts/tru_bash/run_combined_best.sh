...
select_operators=("1" "2" "3" "4" "5" "6" "7")
...
echo "java -Xms500m -Xmx500m gelations.RunPrioritizer $data_format $mut_operator $cross_operator $select_operator $fit_transform $mut_rate $child_rep $pop_size $max_time $max_fitness $max_stag $metric $real_data_path$real_config${real_types[1]} $real_data_path$real_config${real_types[0]} $real_data_path$real_config${real_types[2]} $real_output_path_best$master_datafile${real_types[3]} $rep"
